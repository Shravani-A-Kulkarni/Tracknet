class NumberPlateDetectionProcessor {
  constructor() {
    this.baseUrl = "http://localhost:5000/api";
    this.controller = null;
    this.modelName = "number_plate_detection"; // Fixed model for this page
  }
  ...
  async processVideo(videoFile) {
    ...
    try {
      ...
      console.log(`⏳ Starting number plate detection...`);
      ...
      if (data.success && data.output_file) {
        ...
        return {
          ...
          message:
            data.message || "Number plate detection completed successfully!",
        };
      } else {
        return {
          success: false,
  ...
  showLoading(show) {
    let loader = document.getElementById("number-plate-loading-indicator");
    if (!loader && show) {
      loader = document.createElement("div");
      loader.id = "number-plate-loading-indicator";
      loader.innerHTML = `
        <div style="display: flex; align-items: center; gap: 10px;">
          <i class="fas fa-car fa-spin"></i>
          <span>Detecting number plates... Please wait</span>
        </div>
      `;
  ...
  cancelProcessing() {
    if (this.controller) {
      this.controller.abort();
      this.showLoading(false);
      console.log("❌ Number plate detection cancelled");
    }
  }
}

// Initialize Processor
const numberPlateDetectionProcessor = new NumberPlateDetectionProcessor();

// DOM Content Loaded
document.addEventListener("DOMContentLoaded", () => {
  initializeNumberPlateDetectionForm();
});

function initializeNumberPlateDetectionForm() {
  const videoFileInput = document.getElementById("videoFile");
  const fileNameDisplay = document.getElementById("fileNameDisplay");
  const uploadForm = document.getElementById("videoUploadForm");

  if (!uploadForm) {
    console.error("❌ Number plate detection upload form not found!");
    return;
  }

  const analyzeButton = uploadForm.querySelector('button[type="submit"]');
  let cancelButton = document.getElementById("numberPlateCancelButton");

  // Create cancel button if it doesn't exist
  if (!cancelButton && analyzeButton) {
    cancelButton = document.createElement("button");
    cancelButton.id = "numberPlateCancelButton";
    ...
    cancelButton.addEventListener("click", () => {
      numberPlateDetectionProcessor.cancelProcessing();
  ...
  uploadForm.addEventListener("submit", async (e) => {
    ...
    if (analyzeButton) {
      ...
      analyzeButton.innerHTML =
        '<i class="fas fa-spinner fa-spin"></i> Detecting Number Plates...';
    }
    if (cancelButton) {
    ...
    try {
      console.log("🔄 Starting number plate detection...");
      const result = await numberPlateDetectionProcessor.processVideo(
        videoFile
      );

      if (result.success) {
        console.log("✅ Number plate detection successful!");
        showNumberPlateDetectionResults(result);
        if (fileNameDisplay) {
          fileNameDisplay.textContent =
            "✅ Number plate detection complete! Video ready below.";
          fileNameDisplay.style.color = "#00EAFF";
        }
      } else {
        alert("Number Plate Detection Error: " + result.error);
        if (fileNameDisplay) {
          fileNameDisplay.textContent = "❌ Number plate detection failed";
...
function showNumberPlateDetectionResults(result) {
  ...
  processedVideoContainer.innerHTML = `
    <div style="background: rgba(0, 234, 255, 0.1); padding: 20px; border-radius: 10px; border: 2px solid #00EAFF;">
      <h3 style="color: #00EAFF; margin-bottom: 15px; display: flex; align-items: center; gap: 10px;">
        <i class="fas fa-check-circle"></i>
        Number Plate Detection Complete!
      </h3>
      <div style="display: flex; flex-direction: column; gap: 15px;">
        <video controls width="100%" style="border-radius: 10px; max-width: 800px; background: #000; border: 2px solid #00EAFF;">
          <source src="${result.streamUrl}" type="video/mp4">
          Your browser does not support the video tag.
        </video>
        <div style="display: flex; gap: 10px; flex-wrap: wrap;">
          <a href="${result.downloadUrl}" 
             download="number_plate_detected_video.mp4" 
             style="background: #00EAFF; color: black; padding: 12px 24px; border-radius: 5px; text-decoration: none; display: inline-flex; align-items: center; gap: 8px; font-weight: bold;">
        ...
        <div style="color: #666; font-size: 14px; margin-top: 10px;">
          <i class="fas fa-info-circle"></i> Detected number plates are highlighted with text recognition
        </div>
      </div>
    </div>
  `;
